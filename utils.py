...
class TSAT_parameter():
    ...
    def __init__(self, dataset_name: str):
        self.model_parameters, self.training_parameters, self.testing_parameters = None, None, None
        assert dataset_name is not None  # The name of dataset should not be None, please check dataset_name input correctly
        ...
        elif dataset_name == 'alpha-1':  #
            self.model_parameters = {
                'l_backcast': 150,  # backcast length
                'd_edge': 5,  # edge features (number of IMF used)
                'd_model': 64,  # model hidden layer
                'N': 2,  # number of Self_Attention_Block
                'h': 2,  # Multi-attention heads
    ...
    def parameters(self):
        ...
        """
        return self.model_parameters, self.training_parameters, self.testing_parameters
...
def evaluation(y_true, y_pred, y_graph_name, requirement, data_mean=0, data_std=1):
    ...
    if 'bce' in requirement:
        ...
        loss = F.binary_cross_entropy_with_logits(torch.tensor(y_pred), torch.tensor(y_cal_true),
                                                  reduction='none') * y_mask
...
if __name__ == "__main__":
    year = 2022
    month = "01"
    with open(f'../data-batch/{year}/G/imf_{month}.pkl', 'rb') as f:
        dic = pickle.load(f)
        # print(dic.shape)
        array_to_torch(dic["imf_1_matix"])

    with open(f'../data-batch/{year}/G/adj_{month}.pkl', 'rb') as f:
        arr = pickle.load(f)
        # print(dic.shape)
        array_to_torch(arr)
